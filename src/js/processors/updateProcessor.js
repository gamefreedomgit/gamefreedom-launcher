...
module.exports = {
    ...
    downloadFile: async function(url, path)
    {
        try
        {
            console.log("downloadFile: Requesting file: " + url);

            // Check if file is already downloading
            if (global.queuedDownloads.includes(url) || global.ongoingDownloads.some(e => e.url === url))
                return;
            ...
            await new Promise((resolve, reject) =>
            {
                ...
                fileStream.on("ready", () =>
                {
                    ...
                    global.ongoingDownloads.push(ongoingDownload);
                });
                fileStream.on("finish", () =>
                {
                    // Remove url from ongoing downloads
                    const ongoingDownload = global.ongoingDownloads.find(e => e.url === url);
                    if (ongoingDownload)
                    {
                        global.ongoingDownloads.splice(global.ongoingDownloads.indexOf(ongoingDownload), 1);
                    }

                    delete global.downloadProgresses[url];
                    fileStream.close();
                    resolve();
                });
            });
